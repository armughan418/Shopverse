...
export default function ProductsPage() {
  ...
  const fetchPage = useCallback(async () => {
    try {
      setLoading(true);
      const res = await axios.get(api().getProductsPaged(page, 12));
      if (res.data?.status) {
  ...
  return (
    <div className="min-h-screen bg-orange-50 flex flex-col">
      <div className="max-w-6xl mx-auto p-4 sm:p-6 flex-1 w-full">
        <h1 className="text-3xl font-bold text-orange-700 mb-6">{t("allProducts")}</h1>

        {loading ? (
          <ProductGridSkeleton count={12} />
        ) : products.length === 0 ? (
          <p className="text-center text-gray-600 py-12">{t("noProductsFound")}</p>
        ) : (
          <>
